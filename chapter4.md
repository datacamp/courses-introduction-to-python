...
`@instructions`
- O código para imprimir a altura média já está incluído. Complete o código da altura mediana. Substitua `____` pelo código correto.
- Use `np.std()` na primeira coluna de `np_baseball` para calcular `stddev`. Substitua `____` pelo código correto.
- Os jogadores grandes tendem a ser mais pesados? Use `np.corrcoef()` para armazenar a correlação entre a primeira e a segunda coluna de `np_baseball` em `corr`. Substitua `____` pelo código correto.
