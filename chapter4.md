...
Este ejercicio ya tiene cargadas dos listas: `height_in` y `weight_lb`. Estas contienen la altura y el peso de los jugadores de la MLB en forma de listas normales y corrientes. También tienes a tu disposición dos arrays de `numpy`: `np_weight_lb` y `np_height_in`.
...
En este ejercicio, `baseball` es una lista de listas. La lista principal contiene 4 elementos. Cada uno de estos elementos es una lista que contiene la altura y el peso de 4 jugadores de béisbol, en este orden. En el script, `baseball` ya está codificado.
...
`@hint`
- En el script, `baseball` ya está codificado. Llama a `np.array()` sobre él y almacena el array 2D resultante de `numpy` en `np_baseball`.
...
Llegas a la conclusión de que tiene más sentido reestructurar toda esta información en una matriz `numpy` bidimensional.
...
Los índices antes de la coma se refieren a las filas, mientras que los que van después de la coma se refieren a las columnas. Para segmentar, usamos `:` que, en este ejemplo, le indica a Python que incluya todas las filas.
...
`@instructions`
- Has conseguido obtener los cambios en altura, peso y edad de todos los jugadores de béisbol. Estos datos se proporcionan como una matriz `numpy` 2D, `updated`. Añade `np_baseball` y `updated`, y muestra el resultado por pantalla.
