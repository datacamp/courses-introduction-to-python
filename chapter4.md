...
`@hint`
- Copy the different code chunks and paste them in the shell.  Run the code (hit **Enter** for web users) and see which output matches the one generated by `np.array([True, 1, 2]) + np.array([3, 4, False])`.
