...
`@hint`
- Copia los distintos trozos de código y pégalos en el shell IPython. Pulsa **Intro** para ejecutar el código y ver qué resultado coincide con el generado por `np.array([True, 1, 2]) + np.array([3, 4, False])`.
...
`@instructions`
- Utiliza `np.array()` para crear una matriz de `numpy` 2D a partir de `baseball`. Llámalo `np_baseball`.
...
- `np_baseball.shape` te dará las dimensiones de `np_baseball`. Asegúrate de envolverlo con una llamada a .
