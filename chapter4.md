...
- Verwende [`np.array()`](http://docs.scipy.org/doc/numpy-1.10.0/glossary.html#term-array), um ein `numpy`-Array aus `baseball` zu erstellen. Nenne dieses Array `np_baseball`.
...
- [`np.array()`](http://docs.scipy.org/doc/numpy-1.10.0/glossary.html#term-array) sollte die Eingabe `baseball` entgegennehmen. Weise das Ergebnis des Funktionsaufrufs `np_baseball` zu.
...
`@sct`
```{python}
predef_msg = "Sie müssen die vordefinierten Variablen nicht ändern oder entfernen."
Ex().has_import("numpy")
Ex().check_correct(
...
Du bist ein wahrer Baseballfan. Deswegen rufst du bei der MLB (Major League Baseball) an und erkundigst dich nach weiteren Statistiken zu den Körpergrößen der wichtigsten Spieler. Man sendet dir Daten zu über tausend Spielern, die als normale Python-Liste gespeichert sind: `height_in`. Die Körpergröße ist jeweils in Zoll angegeben. Kannst du ein `numpy`-Array daraus machen und die Maße in Meter umrechnen?
...
- Multipliziere `np_height_in` mit `0.0254`, um alle Größenangaben von Zoll in Meter umzurechnen. Speichere die neuen Werte in einem neuen Array namens `np_height_m`.
- Gib `np_height_m` aus und prüfe, ob die Ausgabe Sinn ergibt.

`@hint`
- Verwende [`np.array()`](http://docs.scipy.org/doc/numpy-1.10.0/glossary.html#term-array) und übergib `height`. Speichere das Ergebnis in `np_height_in`.
...
Ex().check_correct(
  has_printout(1),
  check_object("np_height_m").has_equal_value(incorrect_msg = "Verwenden Sie `np_height_in * 0.0254`, um `np_height_m` zu berechnen.")
)

success_msg("Schön! Im Handumdrehen führt `numpy` Multiplikationen an mehr als 1000 Höhenmessungen durch.")
...
`numpy` eignet sich hervorragend für die Vektorrechnung. Wenn du die Funktionalität mit normalen Python-Listen vergleichst, hat sich jedoch einiges geändert.

Erstens können `numpy`-Arrays keine Elemente mit unterschiedlichen Typen enthalten.
...
Wir haben einige Codezeilen für dich vorbereitet. Probiere sie aus und wähle diejenige, die hierzu passen würde:

```
np.array([True, 1, 2]) + np.array([3, 4, False])
```

Das Paket `numpy` ist bereits als `np` importiert.
...
`@hint`
- Kopiere die verschiedenen Codezeilen in die IPython-Shell. Drücke die **Eingabetaste**, um den Code auszuführen und zu sehen, welche Ausgabe mit der von `np.array([True, 1, 2]) + np.array([3, 4, False])` übereinstimmt.
...
`@sct`
```{python}
msg1 = msg3 = msg4 = "Falsch. Probieren Sie die verschiedenen Codeabschnitte aus und sehen Sie, welcher mit dem Zielcodeabschnitt übereinstimmt."
msg2 = "Großartige Arbeit! `True` wird in 1 umgewandelt, `False` wird in 0 umgewandelt."
...
Für diese Übung wurden bereits die zwei Listen `height_in` und `weight_lb` im Hintergrund für dich geladen. Sie enthalten jeweils die Körpergröße und das Gewicht der Spieler aus der MLB als reguläre Listen. Außerdem sind die zwei `numpy`-Arrays `np_weight_lb` und `np_height_in` für dich vorbereitet.
...
Bevor wir uns mit den eigentlichen MLB-Daten beschäftigen, wollen wir versuchen, ein 2D-`numpy`-Array aus einer kleinen Liste von Listen zu erstellen.
...
`@instructions`
- Verwende [`np.array()`](http://docs.scipy.org/doc/numpy-1.10.0/glossary.html#term-array), um ein 2D-`numpy`-Array aus `baseball` zu erstellen. Nenne es `np_baseball`.
- Gib den Typ von `np_baseball` aus.
- Gib das `shape`-Attribut von `np_baseball` aus. Verwende `np_baseball.shape`.

`@hint`
- `baseball` ist bereits im Skript für dich kodiert. Rufe damit [`np.array()`](http://docs.scipy.org/doc/numpy-1.10.0/glossary.html#term-array) auf und speichere das resultierende 2D-`numpy`-Array in `np_baseball`.
- Kombiniere [`print()`](https://docs.python.org/3/library/functions.html#print) und [`type()`](https://docs.python.org/3/library/functions.html#type) für die zweite Aufgabe.
- `np_baseball.shape` liefert dir die Dimensionen von `np_baseball`. Denk daran, alles in einen [`print()`](https://docs.python.org/3/library/functions.html#print)-Aufruf zu packen!
...
success_msg("Großartig! Sie sind jetzt bereit, die tatsächlichen MLB-Daten in ein 2D-`numpy`-Array zu konvertieren!")
```
...
`@hint`
- `baseball` ist bereits in der Python-Umgebung verfügbar. Rufe damit [`np.array()`](http://docs.scipy.org/doc/numpy-1.10.0/glossary.html#term-array) auf und speichere das resultierende 2D-`numpy`-Array in `np_baseball`.
...
success_msg("Glatt! Zeit, einige Killerfunktionen von mehrdimensionalen `numpy`-Arrays zu zeigen!")
```
...
- Für die letzte Aufgabe brauchst du `[123, 0]`. Vergiss nicht, das Ganze in eine [`print()`](https://docs.python.org/3/library/functions.html#print)-Anweisung zu packen.
...
Ex().check_object('np_weight_lb').has_equal_value(incorrect_msg = "Sie können `np_baseball[:,1]` verwenden, um `np_weight_lb` zu definieren. Dies wird die gesamte erste Spalte auswählen.")
...
- Du möchtest die Maßeinheiten für Körpergröße und Gewicht in metrische Einheiten (Meter bzw. Kilogramm) umrechnen. Im ersten Schritt erstellst du ein `numpy`-Array mit drei Werten: `0.0254`, `0.453592` und `1`. Weise diesem Array den Namen `conversion` zu.
...
success_msg("Großartige Arbeit! Beachten Sie, wie Sie mit sehr wenig Code alle Werte in Ihrer `numpy`-Datenstruktur auf eine sehr spezifische Weise ändern können. Dies wird in Ihrer Zukunft als Datenwissenschaftler sehr nützlich sein!")
```
...
- Wenn `numpy` als `np` importiert wird, kannst du mit [`np.mean()`](http://docs.scipy.org/doc/numpy-1.10.0/reference/generated/numpy.mean.html) den Mittelwert eines NumPy-Arrays berechnen. Vergiss nicht den [`print()`](https://docs.python.org/3/library/functions.html#print)-Aufruf!
- Für die letzte Aufgabe brauchst du [`np.median()`](http://docs.scipy.org/doc/numpy-1.10.0/reference/generated/numpy.median.html).
...
Ex().check_object("np_height_in").has_equal_value(incorrect_msg = "Sie können `np_baseball[:,0]` verwenden, um die erste Spalte von `np_baseball` auszuwählen"),
...
success_msg("Eine durchschnittliche Höhe von 1586 Zoll, das klingt nicht richtig, oder? Die Medianhöhe scheint jedoch nicht von den Ausreißern beeinflusst zu sein: 74 Zoll machen vollkommen Sinn. Es ist immer eine gute Idee, sowohl den Median als auch den Mittelwert zu überprüfen, um eine Vorstellung von der Gesamtverteilung des gesamten Datensatzes zu bekommen.")
...
`@instructions`
- Der Code zum Ausgeben der durchschnittlichen Körpergröße ist bereits vorhanden. Vervollständige den Code für den Medianwert der Körpergröße. Ersetze `None` durch den richtigen Code.
- Verwende [`np.std()`](http://docs.scipy.org/doc/numpy-1.10.0/reference/generated/numpy.std.html) mit der ersten Spalte von `np_baseball`, um `stddev` zu berechnen. Ersetze `None` durch den richtigen Code.
- Sind die großen Spieler tendenziell schwerer? Verwende [`np.corrcoef()`](http://docs.scipy.org/doc/numpy-1.10.0/reference/generated/numpy.corrcoef.html), um die Korrelation zwischen der ersten und zweiten Spalte von `np_baseball` in `corr` zu speichern. Ersetze `None` durch den richtigen Code.

`@hint`
- Verwende [`np.median()`](http://docs.scipy.org/doc/numpy-1.10.0/reference/generated/numpy.median.html), um den Median zu berechnen. Achte darauf, zuerst die richtige Spalte auszuwählen!
- Wähle dieselbe Spalte bei der Berechnung der Standardabweichung mit [`np.std()`](http://docs.scipy.org/doc/numpy-1.10.0/reference/generated/numpy.std.html) aus.
- Verwende `np_baseball[:, 0]` und `np_baseball[:, 1]`, um die erste und zweite Spalte auszuwählen, die als Eingaben für [`np.corrcoef()`](http://docs.scipy.org/doc/numpy-1.10.0/reference/generated/numpy.corrcoef.html) dienen.
...
`@sct`
```{python}
msg = "Sie sollten die vordefinierte Variable `avg` nicht ändern oder entfernen."
Ex().check_object("avg", missing_msg=msg).has_equal_value(incorrect_msg=msg)

missing = "Haben Sie `np.median()` verwendet, um den Median zu berechnen?"
incorrect = "Um `med` zu berechnen, übergeben Sie die erste Spalte von `np_baseball` an `numpy.median()`. Das Beispiel von `np.mean()` zeigt, wie es gemacht wird."
Ex().check_correct(
...
missing = "Haben Sie `np.std()` verwendet, um die Standardabweichung zu berechnen?"
incorrect = "Um `stddev` zu berechnen, übergeben Sie die erste Spalte von `np_baseball` an `numpy.std()`. Das Beispiel von `np.mean()` zeigt, wie es gemacht wird."
Ex().check_correct(
...
missing = "Haben Sie `np.corrcoef()` verwendet, um die Korrelation zu berechnen?"
...
incorrect2 = "Um `corr` zu berechnen, sollte das zweite Argument von `np.corrcoef()` die zweite Spalte von `np_baseball` sein. Anstelle von `[:,0]` verwenden Sie diesmal `[:,1]`."
Ex().check_correct(
...
success_msg("Großartig! Zeit, all Ihre neuen Data-Science-Fähigkeiten in der letzten Übung anzuwenden!")
