...
- Verwende `np.array()`, um ein `numpy` Array aus `baseball` zu erstellen. Nenne dieses Array `np_baseball`.
...
- `np.array()` sollte die Eingabe `baseball` übernehmen. Weise das Ergebnis des Funktionsaufrufs `np_baseball` zu.
...
`@hint`
- Verwende `np.array()` und gib es an `height` weiter. Speichere das Ergebnis in `np_height_in`.
...
`@instructions`
- Verwende `np.array()`, um ein 2D `numpy` Array aus `baseball` zu erstellen. Nenne ihn `np_baseball`.
- Gib den Typ von `np_baseball` aus.
- Gib das `shape`-Attribut von `np_baseball` aus. Verwende `np_baseball.shape`.

`@hint`
- `baseball` ist bereits im Skript für dich kodiert. Rufe `np.array()` auf und speichere das resultierende 2D `numpy` Array in `np_baseball`.
- Verwende `print()` in Kombination mit `type()` für die zweite Anweisung.
- `np_baseball.shape` liefert dir die Dimensionen von `np_baseball`. Vergewissere dich, dass du einen `print()` Aufruf einbindest.
...
`@hint`
- `baseball` ist bereits in der Python-Umgebung verfügbar. Rufe `np.array()` auf und speichere das resultierende 2D `numpy` Array in `np_baseball`.
...
- Für die letzte Anweisung verwendest du `[123, 0]`; vergiss nicht, alles in eine `print()` Anweisung zu packen.
...
- Du möchtest die Maßeinheiten für Körpergröße und Gewicht in metrische Einheiten (Meter bzw. Kilogramm) umrechnen. Im ersten Schritt erstellst du ein `numpy`-Array mit drei Werten: `0.0254`, `0.453592` und `1`. Nenne dieses Array `conversion`.
...
- Wenn `numpy` als `np` importiert wird, kannst du `np.mean()` verwenden, um den Mittelwert eines NumPy-Arrays zu ermitteln. Vergiss nicht, einen `print()` Anruf zu tätigen.
- Verwende für die letzte Anweisung `np.median()`.
...
- Benutze `np.std()` in der ersten Spalte von `np_baseball`, um `stddev` zu berechnen. Ersetze `None` durch den richtigen Code.
- Sind die großen Spieler tendenziell schwerer? Verwende `np.corrcoef()`, um die Korrelation zwischen der ersten und zweiten Spalte von `np_baseball` in `corr` zu speichern. Ersetze `None` durch den richtigen Code.

`@hint`
- Verwende `np.median()`, um den Median zu berechnen. Achte darauf, zuerst die richtige Spalte auszuwählen!
- Setze die gleiche Spalte bei der Berechnung der Standardabweichung mit `np.std()` ein.
- Benutze `np_baseball[:, 0]` und `np_baseball[:, 1]`, um die erste und zweite Spalte auszuwählen; diese sind die Eingaben für `np.corrcoef()`.
