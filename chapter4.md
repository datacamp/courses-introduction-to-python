...
In dieser Übung widmen wir uns einer beliebten amerikanischen Sportart: Baseball! In diesem Zusammenhang lernst du die Grundlagen von `numpy` kennen, einem praktischen Paket für Data Science.
...
`@hint`
- Schreibe `import numpy as np`. Jetzt musst du `np.fun_name()` verwenden, wenn du eine Funktion aus `numpy` nutzen willst.
- `np.array()` sollte `baseball` als Eingabe entgegennehmen. Weise das Ergebnis des Funktionsaufrufs `np_baseball` zu.
- Um den Typ einer Variablen `x` auszugeben, schreibst du einfach `print(type(x))`.
...
Angenommen, du bist ein großer Baseballfan. Du beschließt, bei der MLB (Major League Baseball) anzurufen und nach weiteren Statistiken zur Größe der wichtigsten Spieler zu fragen. Man sendet dir Daten zu über tausend Spielern, die als normale Python-Liste gespeichert sind: `height_in`. Die Körpergröße ist jeweils in Zoll angegeben. Kannst du ein `numpy`-Array daraus machen und die Maße in Meter umrechnen?
...
- Um eine Variable `x` auszugeben, schreibst du `print(x)` im Python-Skript.
...
`@hint`
- Denk daran, deine Abfragen bestimmter Elemente in eine `print()`-Funktion zu setzen.
...
In dieser Übung ist `baseball` eine Liste, deren Elemente auch Listen sind. Die Hauptliste enthält vier Elemente. Jedes dieser Elemente ist wiederum eine Liste und enthält jeweils die Körpergröße und das Gewicht (in genau dieser Reihenfolge) von vier Baseballspielern. `baseball` ist bereits für dich im Skript programmiert.
...
`@hint`
- `baseball` ist bereits im Skript für dich programmiert. Rufe `np.array()` dafür auf und speichere das resultierende 2D-`numpy`-Array in `np_baseball`.
- Für die zweite Aufgabe musst du `print()` mit `type()` kombinieren.
- `np_baseball.shape` liefert dir die Dimensionen von `np_baseball`. Denk daran, alles in einen `print()`-Aufruf zu setzen!
...
`@hint`
- `baseball` ist bereits in der Python-Umgebung verfügbar. Rufe `np.array()` dafür auf und speichere das resultierende 2D-`numpy`-Array in `np_baseball`.
...
Wenn dein 2D-`numpy`-Array eine regelmäßige Struktur hat (das heißt, jede Zeile und Spalte hat eine feste Anzahl von Werten), wird der bisher komplizierte Zugriff auf einzelne Elemente ganz einfach. Sieh dir folgenden Code an, in dem die Elemente `"a"` und `"c"` aus einer Liste extrahiert werden, die wiederum aus Listen besteht.
...
`@hint`
- Für die erste Aufgabe brauchst du den Zeilenindex 49. Um genau zu sein, solltest du `[49, :]` verwenden.
- Um die gesamte zweite Spalte auszuwählen, brauchst du `[:, 1]`.
- Nutze `[123, 0]` für die letzte Aufgabe. Vergiss nicht, das Ganze in eine `print()`-Anweisung zu setzen.
...
`np_baseball` ist bereits programmiert. Es handelt sich wieder um ein 2D-`numpy`-Array mit drei Spalten für die Körpergröße (in Zoll), das Gewicht (in Pfund) und das Alter (in Jahren). `baseball` liegt als eine reguläre Liste aus Listen vor. Und bei `updated` handelt es sich um ein 2D-NumPy-Array.

`@instructions`
- Dir liegen die Änderungen von Körpergröße, Gewicht und Alter aller Baseballspieler vor, und zwar als 2D-`numpy`-Array namens `updated`. Füge `np_baseball` und `updated` zusammen und gib das Ergebnis aus.
- Du möchtest die Körpergröße und das Gewicht in metrische Einheiten (Meter bzw. Kilogramm) umrechnen. Im ersten Schritt erstellst du ein `numpy`-Array mit drei Werten: `0.0254`, `0.453592` und `1`. Nenne das Array `conversion`.
...
- Erstelle mit `np.array()`ein `numpy`-Array. Die Eingabe ist eine reguläre Python-Liste mit drei Elementen.
...
Die Baseballdaten liegen als 2D-`numpy`-Array mit 3 Spalten (Körpergröße, Gewicht, Alter) und 1015 Zeilen vor. Der Name dieses `numpy`-Arrays lautet `np_baseball`. Nachdem du die Daten umstrukturiert hast, stellst du jedoch fest, dass einige Größenangaben ungewöhnlich hoch sind. Befolge die Anweisungen und ermittle, welche zusammenfassende Kennzahl am besten geeignet ist, wenn du es mit sogenannten _Ausreißern_ zu tun hast. `np_baseball` ist bereits vorhanden.
...
Das Python-Skript im Editor enthält schon Code, um die informativen zusammenfassenden Kennzahlen auszugeben. `numpy` ist bereits als `np` geladen. Kannst du die Aufgabe abschließen? `np_baseball` ist verfügbar.
