...
`@instructions`
- Der Code zum Ausgeben der durchschnittlichen Körpergröße ist bereits vorhanden. Vervollständige den Code für den Medianwert der Körpergröße. Ersetze `____` durch den richtigen Code.
- Benutze `np.std()` in der ersten Spalte von `np_baseball`, um `stddev` zu berechnen. Ersetze `____` durch den richtigen Code.
- Sind die großen Spieler tendenziell schwerer? Verwende `np.corrcoef()`, um die Korrelation zwischen der ersten und zweiten Spalte von `np_baseball` in `corr` zu speichern. Ersetze `____` durch den richtigen Code.
