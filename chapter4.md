...
`@instructions`
- Utilisez `np.array()` pour créer un tableau 2D `numpy` à partir de `baseball`. Donnez-lui le nom : `np_baseball`.
- Affichez le type de `np_baseball`.
- Affichez l'attribut `shape` de `np_baseball`. Utiliser `np_baseball.shape`.
