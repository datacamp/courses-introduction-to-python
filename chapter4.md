...
Você é um grande fã de beisebol. Você decide ligar para MLB (Major League Baseball) e pedir mais algumas estatísticas sobre a altura dos principais jogadores. Ela transmite dados sobre mais de mil jogadores, que são armazenados como uma lista comum do Python: `height_in`. A altura está expressa em polegadas. Você consegue criar uma matriz do `numpy` com ela e converter a unidade em metros?
...
`@hint`
- Use `np.array()` e passe para `height`. Armazene o resultado em `np_height_in`.
...
Em primeiro lugar, as matrizes do `numpy` não podem conter elementos com tipos diferentes.
...
`@instructions`
- Use `np.array()` para criar uma matriz 2D `numpy` a partir de `baseball`. Chame-a de `np_baseball`.
- Imprima o tipo de `np_baseball`.
- Imprima o atributo `shape` de `np_baseball`. Use `np_baseball.shape`.

`@hint`
- `baseball` já está codificado para você no script. Chame `np.array()` e armazene a matriz 2D `numpy` resultante em `np_baseball`.
- Use `print()` em combinação com `type()` para a segunda instrução.
- `np_baseball.shape` apresenta as dimensões de `np_baseball`. Certifique-se de envolver uma chamada `print()` em torno dele.
...
`@hint`
- `baseball` já está disponível no ambiente Python. Chame `np.array()` e armazene a matriz 2D `numpy` resultante em `np_baseball`.
- `np_baseball.shape` apresenta as dimensões `np_baseball`. Certifique-se de envolver uma chamada `print()`em torno dele.
...
- Para a última instrução, use `[123, 0]`; não se esqueça de envolver tudo em uma instrução `print()`.
...
- `np_baseball * conversion` resolve o problema, sem trabalho extra. Experimente! Certifique-se de envolvê-lo em uma chamada para `print()`.
...
- Se `numpy` for importado como `np`, você poderá usar `np.mean()` para obter a média de uma matriz NumPy. Não se esqueça de incluir uma chamada para `print()`.
- Para a última instrução, use `np.median()`.
...
`@instructions`
- O código para imprimir a altura média já está incluído. Complete o código da altura mediana. Substitua `None` pelo código correto.
- Use `np.std()` na primeira coluna de `np_baseball` para calcular `stddev`. Substitua `None` pelo código correto.
- Os jogadores grandes tendem a ser mais pesados? Use `np.corrcoef()` para armazenar a correlação entre a primeira e a segunda coluna de `np_baseball` em `corr`. Substitua `None` pelo código correto.

`@hint`
- Use o site `np.median()` para calcular a mediana. Lembre-se de selecionar a coluna correta primeiro!
- Subsetar a mesma coluna ao calcular o desvio padrão com `np.std()`.
- Use `np_baseball[:, 0]` e `np_baseball[:, 1]` para selecionar a primeira e a segunda colunas; essas são as entradas para `np.corrcoef()`.
