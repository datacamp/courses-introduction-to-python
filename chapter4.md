...
Eres un gran aficionado al béisbol. Decides llamar a MLB (las Grandes Ligas de Béisbol) y preguntar por más estadísticas sobre la estatura de los principales jugadores. Te facilitan datos sobre más de mil jugadores, almacenados como una lista normal de Python: `height_in`. La altura se expresa en pulgadas. ¿Puedes hacer una matriz de `numpy` y convertir las unidades a metros?
...
En primer lugar, las matrices de `numpy` no pueden contener elementos de distintos tipos.
...
`@hint`
- Copia los distintos fragmentos de código y pégalos en el terminal IPython. Pulsa **Intro** para ejecutar el código y ver qué resultado coincide con el generado por `np.array([True, 1, 2]) + np.array([3, 4, False])`.
...
Este ejercicio ya tiene dos listas, `height_in` y `weight_lb`, cargadas en segundo plano para ti. Contienen la altura y el peso de los jugadores de MLB como listas normales. Este ejercicio también tiene preparadas para ti dos listas de matrices de `numpy`, `np_weight_lb` y `np_height_in`.
...
Antes de trabajar con los datos reales de MLB, vamos a intentar crear una matriz de `numpy` 2D a partir de una pequeña lista de listas.
...
`@instructions`
- El código para imprimir la altura media ya está incluido. Completa el código de la altura media. Sustituye `____` por el código correcto.
- Utiliza `np.std()` en la primera columna de `np_baseball` para calcular `stddev`. Sustituye `____` por el código correcto.
- ¿Los jugadores altos tienden a ser más pesados? Utiliza `np.corrcoef()` para almacenar la correlación entre la primera y segunda columna de `np_baseball` en `corr`. Sustituye `____` por el código correcto.
