...
`@instructions`
- Kombiniere `print()` mit `type()`, um den Datentyp von `var1` auszugeben.
- Verwende `len()`, um die [Länge der Liste](https://docs.python.org/3/library/functions.html#len) `var1` zu ermitteln. Setze alles in einen `print()`-Aufruf, um das Ergebnis auszugeben.
...
`@hint`
- Die Funktion `type()` rufst du wie folgt auf: `type(var1)`.
- Die Funktion `print()` rufst du wie gehabt auf. Setze die Variable, die du ausgeben willst, einfach in Klammern.
...
Vielleicht kennst du schon den Namen einer Python-Funktion, weißt aber noch nicht genau, wie du sie benutzen kannst? Um weitere Informationen zu deiner gewünschten Funktion zu erhalten, musst du ironischerweise eine andere Funktion aufrufen, nämlich `help()`. Speziell in IPython kannst du auch `?` vor dem Funktionsnamen verwenden.
...
`@hint`
- Du addierst `first` und `second`, als ob es zwei Zahlen wären, und weist das Ergebnis `full` zu.
- Verwende `sorted()` mit zwei Eingaben: `full` und `reverse=True`.
- Um eine Variable auszugeben, verwendest du `print()`.
...
Für Zeichenfolgen gibt es eine Reihe von Methoden. Folge den Anweisungen genau, um einige von ihnen kennenzulernen. Wenn du mehr darüber erfahren möchtest, kannst du jederzeit `help(str)` in der IPython-Shell eingeben.

Die Zeichenfolge `place` wurde bereits erstellt, sodass du mit ihr experimentieren kannst.

`@instructions`
- Wende die [Methode](https://docs.python.org/3/library/stdtypes.html#str.upper) `.upper()` auf `place` an und speichere das Ergebnis in `place_up`. Verwende die Syntax für den Aufruf von Methoden, die im vorigen Video vorgestellt wurde.
- Gib `place` und `place_up` aus. Haben sich beide Objekte verändert?
- Gib die Anzahl des Buchstabens „o“ in der Variablen `place` aus, indem du `.count()` für `place` aufrufst und den Buchstaben `'o'` als Input an die Methode übergibst. Es geht hier um die Variable `place`, nicht um das Wort `"place"`!

`@hint`
- Du kannst die Methode `.upper()` für `place` ohne weitere Eingaben aufrufen.
- Um eine Variable `x` auszugeben, schreibst du `print(x)`.
- Denk daran, den Aufruf von `place.count(____)` in eine `print()`-Funktion zu setzen, um das Ergebnis auszugeben.
...
Zeichenfolgen sind nicht die einzigen Python-Typen, für die es spezifische Methoden gibt. Listen, Gleitkommazahlen, Ganzzahlen und boolesche Werte sind ebenfalls Typen, die über zahlreiche nützliche Methoden verfügen. In dieser Übung experimentierst du mit:

- `.index()`, um den Index des ersten Elements einer Liste zu erhalten, das mit dem übergebenen Input übereinstimmt, und
...
`@instructions`
- Verwende die Methode `.index()`, um den Index desjenigen Elements in `areas` zu ermitteln, welches gleich `20.0` ist. Gib den Index aus.
- Rufe `.count()` für `areas` auf, um zu ermitteln, wie oft `9.50` in der Liste vorkommt. Gib dann das Ergebnis aus.

`@hint`
- Um den Index auszugeben, setzt du den Aufruf von `areas.index(___)` in eine `print()`-Funktion.
- Um auszugeben, wie oft ein Element `x` in einer Liste vorkommt, setzt du den Aufruf von `areas.count(___)` in eine `print()`-Funktion.
...
Die meisten Listenmethoden ändern die Liste, für die sie aufgerufen werden. Beispiele sind:

- `.append()`, welche ein Element zu der Liste hinzufügt, für welche die Methode aufgerufen wird,
...
- `.reverse()`, welche die Reihenfolge der Elemente in der Liste [umkehrt](https://docs.python.org/3/library/stdtypes.html#typesseq-mutable), für die sie aufgerufen wird.
...
- Um eine Variable `x` auszugeben, schreibst du `print(x)`.
- Die Methode `.reverse()` erfordert keine weiteren Eingaben. Du nutzt einfach die Punktschreibweise und leere Klammern: `.reverse()`.
- Um eine Variable `x` auszugeben, schreibst du `print(x)`.
...
`@hint`
- Für den selektiven Import nutzt du `from math import pi`.
