...
Utiliza la Shell IPython para abrir la [documentación](https://docs.python.org/3/library/functions.html#pow) en `pow()`. Para ello, escribe `?pow` o `help(pow)` y pulsa **Intro**.
...
Echa un vistazo a la [documentación](https://docs.python.org/3/library/functions.html#sorted) de `sorted()` escribiendo `help(sorted)` en la Shell de IPython.
...
Las cadenas tienen un montón de métodos. Sigue atentamente las instrucciones para descubrir algunos de ellos. Si quieres descubrirlos con más detalle, siempre puedes escribir `help(str)` en el shell IPython.
