...
Python bietet von Haus aus mehrere vordefinierte Funktionen, die dir das Leben als Data Scientist erleichtern. Du kennst bereits zwei solcher Funktionen: [`print()`](https://docs.python.org/3/library/functions.html#print) und [`type()`](https://docs.python.org/3/library/functions.html#type). Es gibt auch Funktionen wie [`str()`](https://docs.python.org/3/library/functions.html#func-str), [`int()`](https://docs.python.org/3/library/functions.html#int), [`bool()`](https://docs.python.org/3/library/functions.html#bool) und [`float()`](https://docs.python.org/3/library/functions.html#float), um zwischen Datentypen zu wechseln. Auch das sind vordefinierte Funktionen.
...
`@instructions`
- Kombiniere [`print()`](https://docs.python.org/3/library/functions.html#print) mit [`type()`](https://docs.python.org/3/library/functions.html#type), um den Typ von `var1` auszugeben.
- Verwende [`len()`](https://docs.python.org/3/library/functions.html#len), um die Länge der Liste `var1` zu ermitteln. Packe dies in einen [`print()`](https://docs.python.org/3/library/functions.html#print)-Aufruf, um das Ergebnis auszugeben.
- Verwende [`int()`](https://docs.python.org/3/library/functions.html#int), um `var2` in eine Ganzzahl umzuwandeln. Speichere das Ergebnis als `out2`.

`@hint`
- Rufe die Funktion [`type()`](https://docs.python.org/3/library/functions.html#type) wie folgt auf: `type(var1)`.
- Rufe die Funktion [`print()`](https://docs.python.org/3/library/functions.html#print) so auf, wie du es schon oft getan hast. Setze die Variable, die du ausgeben willst, einfach in Klammern.
...
int_miss_msg = "Haben Sie [`int()`](https://docs.python.org/3/library/functions.html#int) verwendet, um eine Ganzzahl von `var2` zu erstellen?"
int_incorr_msg = "Haben Sie `var2` an [`int()`](https://docs.python.org/3/library/functions.html#int) übergeben?"
Ex().check_correct(
...
success_msg("Großartige Arbeit! Die `len()`-Funktion ist äußerst nützlich; sie funktioniert auch bei Zeichenketten, um die Anzahl der Zeichen zu zählen!")
```
...
Vielleicht kennst du schon den Namen einer Python-Funktion, weißt aber noch nicht genau, wie du sie benutzen kannst? Dann musst du ironischerweise eine andere Funktion aufrufen, nämlich `help()`, um weitere Informationen zu deiner gewünschten Funktion zu erhalten. Speziell in IPython kannst du auch ein `?` vor dem Funktionsnamen verwenden.
...
Verwende die IPython-Shell, um die Dokumentation zu [`pow()`](https://docs.python.org/3/library/functions.html#pow) anzuzeigen. Gib dazu `?pow` oder `help(pow)` ein und drücke die **Eingabetaste**.
...
Lies dir die Dokumentation zu [`sorted()`](https://docs.python.org/3/library/functions.html#sorted) durch, indem du `help(sorted)` in der IPython-Shell eingibst.
...
Für Zeichenketten gibt es eine Reihe von Methoden. Folge den Anweisungen genau, um einige von ihnen kennenzulernen. Weitere Informationen kannst du jederzeit durch die Eingabe von `help(str)` in der IPython-Shell abrufen.

Die Zeichenkette `place` wurde bereits erstellt, sodass du mit ihr experimentieren kannst.

`@instructions`
- Rufe die [`.upper()`](https://docs.python.org/3/library/stdtypes.html#str.upper)-Methode mit `place` auf und speichere das Ergebnis in `place_up`. Verwende die Syntax für den Aufruf von Methoden, die im vorigen Video vorgestellt wurde.
- Gib `place` und `place_up` aus. Haben sich beide Objekte verändert?
- Gib die Anzahl des Buchstabens o in der Variablen `place` aus, indem du [`.count()`](https://docs.python.org/3/library/stdtypes.html#str.count) mit `place` aufrufst und den Buchstaben `'o'` als Eingabe an die Methode übergibst. Es geht hier um die Variable `place`, nicht um das Wort `"place"`!

`@hint`
- Du kannst die [`.upper()`](https://docs.python.org/3/library/stdtypes.html#str.upper)-Methode mit `place` ohne weitere Eingaben aufrufen.
- Um eine Variable `x` auszugeben, kannst du `print(x)` schreiben.
- Denk daran, den Aufruf von `place.count(____)` in eine [`print()`](https://docs.python.org/3/library/functions.html#print)-Funktion zu packen, damit das Ergebnis ausgegeben wird.
...
Ex().check_correct(
  has_printout(2, not_printed_msg = "Sie haben die Anzahl der o's in `place` richtig berechnet; stellen Sie nun sicher, dass Sie den Aufruf `place.count('o')` in eine `print()`-Funktion einbetten, um das Ergebnis auszugeben."),
  check_function("place.count", signature=False).check_args(0).has_equal_value()
)

success_msg("Schön! Beachten Sie bei den Ausgaben, dass die [upper()](https://docs.python.org/3/library/stdtypes.html#str.upper)-Methode das Objekt, auf dem sie aufgerufen wird, nicht ändert. Dies wird bei Listen in der nächsten Übung anders sein!")
...
- [`.index()`](https://docs.python.org/3/library/stdtypes.html#str.index), um den Indexwert des ersten Elements einer Liste zu erhalten, das mit der Eingabe übereinstimmt, und
- [`.count()`](https://docs.python.org/3/library/stdtypes.html#str.count), um zu zählen, wie häufig ein Element in einer Liste vorkommt.

Du nutzt wieder die Liste `areas` mit den Flächen verschiedener Räumlichkeiten eines Hauses.

`@instructions`
- Verwende die [`.index()`](https://docs.python.org/3/library/stdtypes.html#str.index)-Methode, um den Index des Elements in `areas` zu ermitteln, dessen Wert gleich `20.0` ist. Gib den Index aus.
- Rufe [`.count()`](https://docs.python.org/3/library/stdtypes.html#str.count) mit `areas` auf, um herauszufinden, wie oft `9.50` in der Liste vorkommt. Dann gib das Ergebnis aus.

`@hint`
- Um den Index auszugeben, packe den Aufruf von `areas.index(___)` in eine [`print()`](https://docs.python.org/3/library/functions.html#print)-Funktion.
- Um auszugeben, wie oft ein Element `x` in einer Liste vorkommt, packe den Aufruf von `areas.count(___)` in eine [`print()`](https://docs.python.org/3/library/functions.html#print)-Funktion.
...
- [`.append()`](https://docs.python.org/3/library/stdtypes.html#typesseq-mutable) fügt ein Element zu der Liste hinzu, mit der die Methode aufgerufen wird;
- [`.remove()`](https://docs.python.org/3/library/stdtypes.html#typesseq-mutable) entfernt das erste Element der Liste, das mit der Eingabe übereinstimmt;
- [`.reverse()`](https://docs.python.org/3/library/stdtypes.html#typesseq-mutable) kehrt die Reihenfolge der Elemente in der Liste um, mit der die Methode aufgerufen wird.

Auch hier nutzt du wieder die Liste `areas` mit den Flächen verschiedener Räumlichkeiten eines Hauses.

`@instructions`
- Verwende [`.append()`](https://docs.python.org/3/library/stdtypes.html#typesseq-mutable) zweimal, um die Größe des überdachten Pools und der Garage wieder hinzuzufügen: `24.5` bzw. `15.45`. Achte auf die korrekte Reihenfolge!
- Gib `areas` aus.
- Verwende die [`.reverse()`](https://docs.python.org/3/library/stdtypes.html#typesseq-mutable)-Methode, um die Reihenfolge der Elemente in `areas` umzukehren.
- Gib `areas` erneut aus.

`@hint`
- Für die erste Aufgabe musst du `areas.append(___)` zweimal aufrufen.
- Um eine Variable `x` auszugeben, schreibe einfach `print(x)`.
- Die [`.reverse()`](https://docs.python.org/3/library/stdtypes.html#typesseq-mutable)-Methode erfordert keine weiteren Eingaben. Nutze einfach die Punktschreibweise und leere Klammern: `.reverse()`.
...
success_msg("Schön! Wenn Sie wissen, wie man mit Funktionen aus Paketen umgeht, steht Ihnen die Macht von _vielen_ Python-Programmierern zur Verfügung!")
```
...
Angenommen, du möchtest die Funktion [`inv()`](http://docs.scipy.org/doc/numpy-1.10.0/reference/generated/numpy.linalg.inv.html) verwenden, die im Unterpaket `linalg` des Pakets `scipy` enthalten ist. Du möchtest diese Funktion wie folgt nutzen können:
...
`@hint`
- Teste die verschiedenen Importbefehle in der IPython-Shell, um zu sehen, bei welcher Variante die Zeile `my_inv([[1, 2], [3, 4]])` ohne Fehler läuft. Drücke die **Eingabetaste**, um den eingegebenen Code auszuführen.
