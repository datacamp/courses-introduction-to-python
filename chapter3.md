...
El siempre innovador lenguaje Python ofrece un montón de funciones integradas para facilitarte la vida como científico de datos. Ya conoces dos de estas funciones: `print()` y `type()`. También hay funciones como `str()`, `int()`, `bool()` y `float()` para cambiar entre tipos de datos. Puedes informarte sobre ellos [aquí.](https://docs.python.org/3/library/functions.html) También son funciones integradas.
...
- Utiliza `len()` para obtener la [longitud de la lista](https://docs.python.org/3/library/functions.html#len) `var1`. Envuélvelo en una llamada a `print()` para imprimirlo directamente.
- Utiliza `int()` para convertir `var2` en un [número entero](https://docs.python.org/3/library/functions.html#int). Guarda la salida como `out2`.

`@hint`
- Llama a la función `type()` así: `type(var1)`.
- Llama a `print()` como hiciste tantas veces antes. Sólo tienes que poner entre paréntesis la variable que quieres imprimir.
...
Quizá ya conozcas el nombre de una función de Python, pero aún tienes que averiguar cómo utilizarla. Irónicamente, tienes que pedir información sobre una función con otra función: `help()`. En IPython específicamente, también puedes utilizar `?` antes del nombre de la función.
...
Utiliza IPython Shell para abrir la [documentación](https://docs.python.org/3/library/functions.html#pow) en `pow()`. Para ello, escribe `?pow` o `help(pow)` y pulsa **Intro**.
...
Echa un vistazo a la [documentación](https://docs.python.org/3/library/functions.html#sorted) de `sorted()` escribiendo `help(sorted)` en la Shell ython IP.
...
Las cadenas vienen con un montón de métodos. Sigue atentamente las instrucciones para descubrir algunas de ellas. Si quieres descubrirlos con más detalle, siempre puedes escribir `help(str)` en la Shell ython IP.
...
- Asegúrate de envolver tu llamada a `place.count(____)` en una función `print()` para poder imprimirla.
...
success_msg("¡Bien hecho! Observa en las impresiones que el método `upper()` no cambia el objeto sobre el que se llama. ¡Esto será diferente para las listas en el próximo ejercicio!")
...
`@hint`
- Para imprimir el índice, envuelve la llamada a `areas.index(___)` en una función `print()`.
- Para imprimir el número de veces que aparece un elemento `x` en la lista, envuelve la llamada a `areas.count(___)` en una función `print()`.
...
- `.append()`que añade un elemento a la lista a la que se llama,
- `.remove()`que [elimina](https://docs.python.org/3/library/stdtypes.html#typesseq-mutable) el primer elemento de una lista que coincida con la entrada, y
- `.reverse()`que [invierte](https://docs.python.org/3/library/stdtypes.html#typesseq-mutable) el orden de los elementos de la lista a la que se llama.

Trabajarás en la lista con la superficie de las distintas partes de la casa: `areas`.

`@instructions`
- Utiliza `.append()` dos veces para añadir de nuevo el tamaño de la casa de la piscina y del garaje: `24.5` y `15.45`, respectivamente. Asegúrate de añadirlos en este orden.
- Imprimir `areas`
...
success_msg("¡Bien hecho! Si sabes cómo manejar funciones de paquetes, el poder de muchos programadores de Python está a tu alcance.")
...
`@hint`
- Prueba las distintas sentencias import en el intérprete de comandos ython IPy comprueba cuál de ellas hace que la línea `my_inv([[1, 2], [3, 4]])` se ejecute sin errores. Pulsa **intro** para ejecutar el código que has escrito.
