...
Vous connaissez peut-être déjà le nom d'une fonction Python, mais vous ne savez pas encore comment l'utiliser. Ironiquement, vous devez demander des informations sur une fonction à l'aide d'une autre fonction : `help()`. Dans IPython, vous pouvez également utiliser `?` avant le nom de la fonction.
...
Consultez la [documentation de](https://docs.python.org/3/library/functions.html#sorted) `sorted()` en tapant `help(sorted)` dans le shell IPython.
...
Les chaînes de caractères sont accompagnées d'un certain nombre de méthodes. Suivez attentivement les instructions pour découvrir certaines d'entre elles. Si vous souhaitez les découvrir plus en détail, vous pouvez toujours taper `help(str)` dans le shell IPython.
...
- Affichez `place` et `place_up`. Les deux ont-ils changé ?
...
patt = "N'oubliez pas d'afficher `%s`."
...
Ex().check_correct(
  has_printout(2, not_printed_msg = "Vous avez bien calculé le nombre de 'o' dans `place`; assurez-vous maintenant d'envelopper l'appel `place.count('o')` dans une fonction `print()` pour afficher le résultat."),
...
Pour référence, `**` est le symbole de l'exponentiation. Par exemple, `3**4` est `3` à la puissance de `4` et donnera `81`.
...
Ex().multi(
  ...
  has_printout(1, not_printed_msg = "__JINJA__:Gardez `{{sol_call}}` pour afficher la surface.")
)
...
`@hint`
- Essayez les différentes instructions d'importation dans le shell IPython et voyez laquelle permet à la ligne `my_inv([[1, 2], [3, 4]])` de s'exécuter sans erreur. Appuyez sur la touche **Entrée** pour exécuter le code que vous avez saisi.
