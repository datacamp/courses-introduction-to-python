...
C'est maintenant à votre tour de vous exercer en écrivant vous-même du code.

`@instructions`
- Imprimez le résultat de la soustraction de `5` à `5` sous `# Subtraction` en utilisant `print()`.
- Imprimez le résultat de la multiplication de `3` par `5` sous `# Multiplication`.
...
N'oubliez pas que `=` en Python signifie _affectation_, il ne teste pas l'égalité ! Essayez-le dans l'exercice en remplaçant `____` par votre code.
...
- Multipliez `monthly_savings` par `num_months` et affectez-le à `new_savings`.
...
Ex().check_object("new_savings").has_equal_value(incorrect_msg = "Avez-vous utilisé les bonnes variables et symboles pour multiplier ? `monthly_savings * num_months` était attendu mais quelque chose d'autre a été obtenu.")
# Ex().check_object("total_savings").has_equal_value(incorrect_msg = "Avez-vous utilisé les bonnes variables et symboles pour additionner ? `savings + new_savings` était attendu mais quelque chose d'autre a été obtenu.")
...
- `int`, ou entier : un nombre sans partie fractionnaire. `savings` La valeur de `100`, est un exemple de nombre entier.
...
- `str` ou string : un type de variable qui représente du texte. Vous pouvez utiliser des guillemets simples ou doubles pour construire une chaîne de caractères.
...
`@instructions`
- Créez un nouveau flottant, `half`, avec la valeur `0.5`.
- Créez une nouvelle chaîne, `intro`, avec la valeur `"Hello! How are you?"`.
- Créez un nouveau booléen, `is_good`, avec la valeur `True`.

`@hint`
- Pour créer une variable en Python, utilisez `=`. Veillez à entourer votre chaîne de guillemets simples ou doubles.
...
Les variables sont de différents types en Python. Vous pouvez voir le type d'une variable en utilisant `type()`. Par exemple, pour voir le type de `a`, exécutez : `type(a)`.

Les différents types de variables se comportent différemment en Python. Lorsque vous additionnez deux chaînes de caractères, par exemple, vous n'obtiendrez pas le même comportement que lorsque vous additionnez deux entiers ou deux booléens.

Il est temps pour vous de le tester.
...
Ex().multi(
    ...
    has_printout(0, not_printed_msg = "__JINJA__:Utilisez `{{sol_call}}` pour afficher le type de `total_savings`.")
)
```
