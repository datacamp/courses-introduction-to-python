...
Dirígete al código y pulsa el botón Ejecutar código para ver el resultado.

`@instructions`
- Pulsa el botón de ejecutar código para ver la salida de `print(5 / 8)`.

`@hint`
- Ejecuta primero el código antes de enviar tu respuesta para que tengas tiempo de explorar el resultado.
...
También puedes añadir **comentarios** a tus scripts de Python. Los comentarios son importantes para que tú y los demás podáis entender de qué trata tu código y no se ejecutan como código Python.

Empiezan con la etiqueta `#`. Mira el comentario en el editor, `# Division`; ¡ahora te toca a ti añadir un comentario!

`@instructions`
- Sustituye `____` por el comentario 
```
# Addition
```

`@hint`
- Si dejas `____` se producirá un error, así que asegúrate de eliminarlo. Incluye `# Addition` justo encima de `print(7 + 10)`.
...
`@sct`
```{python}
Ex().has_code("#\s*(\w+)[\s.!?]*print\s*\(\s*7", not_typed_msg = "Asegúrate de agregar el comentario justo antes de `print(7 + 10)`.")
success_msg("¡Genial!")
```
...
`@instructions`
- Imprime la suma de `4 + 5`.
- Imprime el resultado de restar `5` a `5`.
- Imprime el resultado de multiplicar `3` por `5`.
- Imprime el resultado de dividir `10` entre `2`.
...
En Python, una variable te permite referirte a un valor con un nombre. Para crear una variable `x` con un valor de `5`, se utiliza `=`, como en este ejemplo:
...
Recuerda, `=` en Python significa _asignación_, ¡no comprueba la igualdad! Pruébalo en el ejercicio sustituyendo `____` por tu código.
...
- ¡Ten cuidado de escribir correctamente las variables!
...
Ex().check_object("new_savings").has_equal_value(incorrect_msg = "¿Usaste las variables y símbolos correctos para multiplicar? Se esperaba `monthly_savings * num_months` pero se obtuvo otra cosa.")
# Ex().check_object("total_savings").has_equal_value(incorrect_msg = "¿Usaste las variables y símbolos correctos para sumar? Se esperaba `savings + new_savings` pero se obtuvo otra cosa.")
...
En el ejercicio anterior, has trabajado con el tipo de datos entero de Python:

- `int`o entero: un número sin parte fraccionaria. `savings`, con el valor `100`, es un ejemplo de número entero.

Además de los tipos de datos numéricos, hay otros tres tipos de datos muy comunes:

- `float`o coma flotante: un número que tiene una parte entera y otra fraccionaria, separadas por un punto. `1.1`, es un ejemplo de flotante.
- `str`o cadena: un tipo para representar texto. Puedes utilizar comillas simples o dobles para construir una cadena.
- `bool`o booleano: un tipo para representar valores lógicos. Sólo puede ser `True` o `False` (¡las mayúsculas son importantes!).
...
`@hint`
- Para crear una variable en Python, utiliza `=`. Asegúrate de encerrar tu cadena entre comillas simples o dobles.
- En Python sólo existen dos valores booleanos: `True` y `False`. `TRUE`, `true`, `FALSE`, `false` y otras versiones no serán aceptadas.
...
`@sct`
```{python}
Ex().check_object("half").has_equal_value(incorrect_msg = "¿Guardaste el flotante, `0.5` en `half`?")
...
En Python existen distintos tipos de variables. Puedes ver el tipo de una variable utilizando `type()`. Por ejemplo, para ver el tipo de `a`, ejecuta: `type(a)`.
...
Ex().multi(
    ...
    has_printout(0, not_printed_msg = "__JINJA__:Usa `{{sol_call}}` para imprimir el tipo de `total_savings`.")
)
```
...
success_msg("Bien. Observa cómo `intro + intro` hace que `\"Hello! How are you?\"` y `\"Hello! How are you?\"` se peguen juntos.")
```
