...
Ahora te toca a ti practicar escribiendo algo de código.

`@instructions`
- Imprime el resultado de restar `5` a `5` bajo `# Subtraction` utilizando `print()`.
- Imprime el resultado de multiplicar `3` por `5` bajo `# Multiplication`.

`@hint`
- Tendrás que utilizar `print()` para generar un resultado.
...
`@sct`
```{python}
Ex().check_object("half").has_equal_value(incorrect_msg = "¿Guardaste el número decimal, `0.5` en `half`?")
...
Ex().check_object("is_good").has_equal_value(incorrect_msg = "¿Pusiste en mayúscula el valor booleano? Recuerda que no necesitas usar comillas aquí.")
...
Ex().multi(
    check_object("doubleintro").has_equal_value(incorrect_msg  = "¿Has guardado el resultado de `intro + intro` en `doubleintro`?"),
