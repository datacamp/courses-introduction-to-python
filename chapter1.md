...
Jetzt bist du an der Reihe zu üben, indem du selbst etwas Code schreibst.

`@instructions`
- Drucke das Ergebnis der Subtraktion von `5` von `5` unter `# Subtraction` mit `print()` aus.
- Drucke das Ergebnis der Multiplikation von `3` mit `5` unter `# Multiplication` aus.

`@hint`
- Du musst `print()` verwenden, um eine Ausgabe zu erzeugen.
- Du kannst mit `-` subtrahieren und mit `*` multiplizieren.
...
success_msg("Das ist korrekt! Python kann dir helfen, die Mathematik zu erledigen, eine Eigenschaft, die nützlich sein wird, wenn wir unsere Datenfähigkeiten weiterentwickeln.")
```
...
Ex().check_object("intro").has_equal_value(incorrect_msg = "Hmm, irgendetwas stimmt mit deiner `intro`-Variable nicht. Überprüfe die Schreibweise und stelle sicher, dass du Anführungszeichen verwendet hast.")

Ex().check_object("is_good").has_equal_value(incorrect_msg = "Hast du den booleschen Wert großgeschrieben? Denk daran, dass du hier keine Anführungszeichen verwenden musst.")
...
Ex().multi(
    ...
    has_printout(0, not_printed_msg = "__JINJA__:Verwende `{{sol_call}}`, um den Typ von `total_savings` auszugeben.")
)
```
