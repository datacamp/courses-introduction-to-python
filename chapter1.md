...
`@hint`
- Führe den Code aus, bevor du deine Antwort abschickst, damit du dich mit der Ausgabe vertraut machen kannst.
...
Python eignet sich perfekt für grundlegende Berechnungen. Möglich sind Addition, Subtraktion, Multiplikation und Division.

Der Code im Skript enthält einige Beispiele.

Kannst du den Code in der Übung vervollständigen?

`@instructions`
- Gib das Ergebnis der Subtraktion – `5`minus`5` – im Abschnitt `# Subtraction` mit `print()` aus.
- Gib das Ergebnis der Multiplikation – `3`multipliziert mit`5` – im Abschnitt `# Multiplication` aus.
...
Variablen ermöglichen es dir in Python, mittels Namen auf bestimmte Werte zu verweisen. Um eine Variable `x` mit dem Wert `5` zu deklarieren, verwenden wir `=` wie in diesem Beispiel:
...
Wie viel Geld sparst du in vier Monaten, wenn du jeden Monat $10 zurücklegst?
...
- `int` steht für „integer“: eine Ganzzahl ohne Nachkommateil. Die Variable `savings` mit dem Wert `100` ist ein Beispiel für eine Ganzzahl.

Neben den numerischen Datentypen gibt es drei weitere häufig genutzte Datentypen:

- `float` steht für „floating point“: eine Gleitkommazahl, die einen ganzzahligen Teil und einen Nachkommateil hat, der in Python durch einen Punkt abgetrennt wird. `1.1` ist ein Beispiel für eine Gleitkommazahl.
- `str` steht für „string“: eine Zeichenfolge zur Darstellung von Text. Du kannst einfache oder doppelte Anführungszeichen verwenden, um eine Zeichenfolge zu erstellen.
...
- Deklariere eine neue Zeichenfolge `intro` mit dem Wert `"Hello! How are you?"`.
- Deklariere einen neuen booleschen Wert `is_good` mit dem Wert `True`.

`@hint`
- Um eine Variable in Python zu deklarieren und ihr einen Wert zuzuweisen, nutzen wir `=`. Achte darauf, dass du deine Zeichenfolge in einfache oder doppelte Anführungszeichen einschließt.
...
In Python gibt es verschiedene Variablentypen. Mit `type()` ermittelst du den Typ einer Variablen. Den Typ von `a` ermittelst du zum Beispiel mit: `type(a)`.

Die verschiedenen Datentypen verhalten sich in Python unterschiedlich. Beispielsweise läuft die Addition von zwei Zeichenfolgen anders ab als die Addition von zwei Ganzzahlen oder zwei booleschen Werten.
...
- Um eine Variable `x` auszugeben, schreibst du `print(x)` im Skript.
