...
Nachdem du die Körpergrößen deiner Familienmitglieder erfasst hast, möchtest du nun einige Informationen zu deinem Wohnhaus sammeln. Die Flächen der verschiedenen Räumlichkeiten des Hauses werden in der Übung in separaten Variablen gespeichert.
...
Obwohl es unüblich ist, kann eine Liste gleichzeitig verschiedene Python-Datentypen wie Zeichenfolgen, Gleitkommazahlen und boolesche Werte enthalten.
...
Ein Teil des Codes wurde schon vorbereitet, damit du direkt loslegen kannst. Aber Achtung! `"bathroom"` ist eine Zeichenfolge, während `bath` eine Variable ist und die Gleitkommazahl `9.50` repräsentiert, die du zuvor angegeben hast.

`@instructions`
- Ergänze den fehlenden Code, der die Liste `areas` erstellt. Strukturiere die Liste so, dass sie zuerst den Namen jedes Raums als Zeichenfolge und dann die zugehörige Fläche enthält. Mit anderen Worten: Füge die Zeichenfolgen `"hallway"`, `"kitchen"` und `"bedroom"` an den richtigen Stellen ein.
...
- Eine Zeichenfolge muss in Anführungszeichen (`""`) gesetzt werden.
...
Anstatt eine Liste mit einem Mix aus Zeichenfolgen und Gleitkommazahlen für die Namen und Flächen der Räumlichkeiten im Haus zu erstellen, kannst du auch eine Liste erstellen, die weitere Listen enthält.

Zur Erinnerung: `"hallway"` ist eine Zeichenfolge, während `hall` eine Variable ist und die Gleitkommazahl `11.25` repräsentiert, die du zuvor angegeben hast.
...
- Um eine Variable `x` auszugeben, schreibst du `print(x)` auf eine neue Zeile.
...
Erinnerst du dich an die Liste `areas` von vorhin, die sowohl Zeichenfolgen als auch Gleitkommazahlen enthielt? Sie ist bereits im Skript deklariert. Kannst du den richtigen Code hinzufügen, um auf die gewünschten Elemente der Python-Liste zuzugreifen?
...
`@hint`
- Um das zweite Element einer Liste `x` auszuwählen, verwendest du `x[1]`.
- Um das letzte Element einer Liste `x` auszuwählen, verwendest du `x[-1]`.
- Denk daran, deine Elementabfrage in einen `print()`-Aufruf zu setzen.
- Die Zahl, die für die Fläche des Wohnzimmers steht, ist das sechste Element der Liste. Du brauchst also den Index `[5]`. `area[4]` würde die Zeichenfolge anzeigen.
...
`@hint`
- Um die ersten sechs Elemente einer Liste zu erhalten, verwendest du eckige Klammern, also `[0:6]`.
- Um alle Elemente einer Liste `l` bis auf die ersten fünf Elemente zu erhalten, verwendest du `l[5:]`.
- Um `downstairs` und `upstairs` auszugeben, fügst du zwei `print()`-Aufrufe hinzu.
...
`@hint`
- Lass uns Schritt für Schritt an diese Aufgabe herangehen. Zuerst benötigst du das letzte Element der Liste `["bathroom", 9.50]`. Zur Erinnerung: Der Index des letzten Elements ist `-1`.
...
`@instructions`
- Aktualisiere die Fläche des Badezimmers von `9.50` auf `10.50` Quadratmeter. Nutze dabei den negativen Index.
- Die Liste `areas` soll ansprechender klingen. Ändere `"living room"` in `"chill zone"`. Verwende diesmal nicht den negativen Index.

`@hint`
- Um die Fläche des Badezimmers zu aktualisieren, greifst du auf das entsprechende Element in der Liste zu. (Tipp: Es ist das letzte!)
- Den Wert ersetzt du dann durch die neue Flächenangabe, indem du sie diesem Element zuweist.
- Dieselben Schritte führst du aus, um den Namen `"living room"` zu aktualisieren, der den Index 4 hat.
...
- Erweitere die Liste `areas_1` nun mit den Daten zu deiner Garage. Füge die Zeichenfolge `"garage"` und die Gleitkommazahl `15.45` hinzu. Nenne die resultierende Liste `areas_2`.

`@hint`
- Orientiere dich bei dieser Übung am Codebeispiel. `x` entspricht hier `areas` und `["e", "f"]` entspricht `["poolhouse", 24.5]`.
- Um weitere Elemente zu `areas_1` hinzuzufügen, benutzt du `areas_1 + ["element", 123]`.
...
Wie sich herausstellt, ist dein Lottogewinn nicht so hoch wie erwartet. Nun sieht es so aus, als könntest du den überdachten Pool doch nicht bauen. Diese Daten gehören also nicht mehr in die Liste. Mit anderen Worten: Du musst die entsprechende Zeichenfolge und zugehörige Gleitkommazahl aus der Liste `areas` entfernen.

`@instructions`
- Lösche die Zeichenfolge und zugehörige Gleitkommazahl für `"poolhouse"` aus deiner `areas`-Liste.
- Gib die aktualisierte Liste `areas` aus.

`@hint`
- Um zwei Elemente zu löschen, musst du `del` zweimal verwenden. Achte dabei auf die korrekten Indexwerte!
...
In dieser Codeversion wird das erste Element der Liste `areas_copy` geändert und dann die Liste `areas` ausgegeben. Wenn du den Code ausführst, siehst du, dass diese Änderung auch in der Liste `areas` auftaucht, obwohl du `areas_copy` geändert hast. Das liegt daran, dass `areas` und `areas_copy` auf dieselbe Liste verweisen.
...
`@hint`
- Du musst die Zuweisung `areas_copy = areas` ändern. Anstelle von `areas` kannst du auch `list(areas)` oder `areas[:]` zuweisen.
