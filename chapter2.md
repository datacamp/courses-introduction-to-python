...
Depois de medir a altura de sua família, você decide coletar algumas informações sobre a casa em que está morando. As áreas das diferentes partes de sua casa são armazenadas em variáveis separadas no exercício.
...
- Lembre-se de incluir uma vírgula `,` após cada sub-lista.
...
- Torne a lista `areas` mais moderna! Altere `"living room"` para `"chill zone"`. Dessa vez, você não deve usar a indexação negativa.
...
Se você pode alterar os elementos de uma lista, com certeza quer poder adicionar elementos a ela, certo? Você pode usar o operador `+`:
