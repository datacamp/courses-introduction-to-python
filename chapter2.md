...
description: >-
  ...
  trabajar eficazmente con grandes cantidades de datos.
attachments:
...
`@instructions`
- Crea una lista, `areas`, que contenga el área del pasillo (`hall`), la cocina (`kit`), el salón (`liv`), el dormitorio (`bed`) y el cuarto de baño (`bath`), en este orden. Utiliza las variables predefinidas.
...
- No necesitas utilizar comillas cuando almacenes variables dentro de una lista.
...
Se te ha proporcionado parte del código para que puedas empezar. ¡Presta atención aquí! `"bathroom"` es una cadena, mientras que `bath` es una variable que representa el `9.50` flotante que especificaste antes.
...
- Vuelve a imprimir `areas`; ¿es esta vez la impresión más informativa?

`@hint`
- Los cuatro primeros elementos de la lista `areas` se codifican como `["hallway", hall, "kitchen", kit, ...`.
...
predef_msg = "¡No elimines ni edites las variables predefinidas!"
areas_msg = "No asignaste el valor correcto a `areas`. Revisa nuevamente las instrucciones. Asegúrate de colocar el nombre de la habitación antes de la variable que contiene el área cada vez. ¡El orden importa aquí! ¡Cuidado con los errores tipográficos!"
...
success_msg("¡Bien hecho! Esta lista contiene tanto cadenas como flotantes, pero eso no es un problema para Python!")
```
...
Recuerda: `"hallway"` es una cadena, mientras que `hall` es una variable que representa el `11.25`flotante que especificaste antes.
...
success_msg("¡Genial! ¡Prepárate para aprender sobre la segmentación de listas!")
```
...
Crear subconjuntos de listas de Python es pan comido. Toma como ejemplo el código siguiente, que crea una lista `x` y luego selecciona "b" de ella. Recuerda que es el segundo elemento, por lo que tiene índice 1. También puedes utilizar la indexación negativa.
...
¿Recuerdas la lista `areas` de antes, que contenía tanto cadenas como flotantes? Su definición ya está en el guión. ¿Puedes añadir el código correcto para crear un subconjunto de Python?

`@instructions`
- Imprime el segundo elemento de la lista `areas` (tiene el valor `11.25`).
- Crea subconjunto e imprime el último elemento de `areas`, siendo `9.50`. ¡Utilizar un índice negativo tiene sentido en este caso!
- Selecciona el número que representa la superficie de la sala de estar (`20.0`) e imprímelo.
...
- ¡El número que representa la superficie de la sala de estar es el 6.º elemento de la lista, así que aquí necesitarás `[5]`. ¡`area[4]` mostraría la cadena!
...
Seleccionar valores individuales de una lista es sólo una parte de la historia. También es posible _segmentar_ tu lista, lo que significa seleccionar varios elementos de tu lista. Utiliza la siguiente sintaxis:

```
my_list[start:end]
```

El índice `start` se incluirá, mientras que el índice `end` _no_. Sin embargo, también es posible no especificar estos índices. Si no especificas el índice `start`, Python deduce que quieres empezar la segmentación al principio de la lista.

`@instructions`
- Utiliza la segmentación para crear una lista, `downstairs`, que contenga los 6 primeros elementos de `areas`.
- Crea `upstairs`, como último elemento `4` de `areas`. Esta vez, simplifica la segmentación omitiendo el índice `end`.
...
patt = "`%s` es incorrecto. Usa `areas[%s]` y el corte para seleccionar los elementos que deseas, o algo equivalente."
...
Para crear un subconjunto de listas de listas, puedes utilizar la misma técnica que antes: los corchetes. Esto sería algo así para una lista, `house`:
...
`@instructions`
- Crea subconjunto de la lista `house` para obtener el `9.5` flotante.
...
Para sustituir elementos de la lista, crear un subconjunto de la lista y asignar nuevos valores al subconjunto. Puedes seleccionar elementos sueltos o cambiar porciones enteras de la lista a la vez.

En este ejercicio y en los siguientes, seguirás trabajando con la lista `areas` que contiene los nombres y las zonas de las distintas habitaciones de una casa.

`@instructions`
- Actualiza la superficie del cuarto de baño para que sea `10.50` metros cuadrados en lugar de `9.50` utilizando la indexación negativa.
- ¡Haz que la lista `areas` esté más de moda! Cambia `"living room"` por `"chill zone"`. Esta vez no utilices la indexación negativa.

`@hint`
- Para actualizar la zona del cuarto de baño, identifica el subconjunto de la zona del cuarto de baño (¡es el último elemento de la lista!).
- A continuación, sustituye el valor por la nueva zona de baño asignándolo a este subconjunto.
...
Ex().check_correct(
  check_object('areas').has_equal_value(incorrect_msg = 'Tus cambios en `areas` no resultaron en la lista correcta. ¿Estás seguro de que usaste las operaciones de subconjunto correctas? En caso de duda, ¡puedes usar una pista!'),
  multi(
...
success_msg('¡Genial! Como mostró el ejemplo de código, también puedes dividir una lista y reemplazarla con otra lista para actualizar múltiples elementos en un solo comando.')
...
Acabas de ganar la lotería, ¡impresionante! Decides construir una caseta de la piscina y un garaje. ¿Puedes añadir la información a la lista `areas`?
...
- Amplía aún más `areas_1` añadiendo datos sobre tu garaje. Añade la cadena `"garage"` y el `15.45` flotante. Nombra la lista resultante `areas_2`.
...
Por desgracia, la cantidad que ganaste con la lotería no es tan grande después de todo y parece que no se va a hacer la casa de la piscina. Tendrás que eliminarla de la lista. Decides eliminar la cadena y el flotante correspondientes de la lista `areas`.

`@instructions`
- Elimina la cadena y el flotante de `"poolhouse"` de tu lista `areas`.
- Imprime la lista `areas` actualizada.

`@hint`
- Tendrás que utilizar `del` dos veces para eliminar dos elementos. Sin embargo, ¡ten cuidado al cambiar los índices!
...
Actualmente, se cambia el primer elemento de la lista `areas_copy` y se imprime la lista `areas`. Si pulsas el botón de ejecutar código verás que, aunque has cambiado `areas_copy`, el cambio también tiene efecto en la lista `areas`. Eso es porque `areas` y `areas_copy` apuntan a la misma lista.

Si quieres evitar que los cambios en `areas_copy` también tengan efecto en `areas`, tendrás que hacer una copia más explícita de la lista `areas` con `list()` o utilizando `[:]`.

`@instructions`
- Cambia el segundo comando, que crea la variable `areas_copy`, de forma que `areas_copy` sea una copia explícita de `areas`. Después de tu edición, los cambios realizados en `areas_copy` no deberían afectar a `areas`. Envía la respuesta para comprobarlo.
...
imsg = "Asegúrate de editar SOLO la copia, no la lista original `areas`. Revisa la descripción del ejercicio si no estás seguro de cómo crear una copia."
Ex().check_correct(
