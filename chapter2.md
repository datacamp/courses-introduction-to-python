...
areas_msg = "Définissez `areas` comme la liste contenant toutes les variables de surface, dans le bon ordre : `[hall, kit, liv, bed, bath]`. Faites attention aux fautes de frappe. La liste ne doit rien contenir d'autre !"
...
Bien que cela ne soit pas vraiment courant, une liste peut également contenir un mélange de types Python, y compris des chaînes de caractères, des flottants et des booléens.
...
Une partie du code vous a été fournie pour vous aider à démarrer. Attention ! `"bathroom"` est une chaîne de caractères, tandis que `bath` est une variable qui représente le flottant `9.50` que vous avez spécifié plus tôt.

`@instructions`
- Terminez le code qui crée la liste `areas`. Construisez la liste de manière à ce qu'elle contienne d'abord le nom de chaque pièce sous forme de chaîne de caractères, puis sa superficie. En d'autres termes, ajoutez les chaînes `"hallway"`, `"kitchen"` et `"bedroom"` aux endroits appropriés.
...
- Une chaîne de caractères devra être mise entre guillemets `""`.
...
success_msg("Bien joué ! Cette liste contient à la fois des chaînes de caractères et des flottants, mais ce n'est pas un problème pour Python !")
```
...
Rappelez-vous : `"hallway"` est une chaîne de caractères, tandis que `hall` est une variable qui représente le flottant `11.25` que vous avez spécifié plus tôt.
...
- Afficher `house` ; cette façon de structurer vos données est-elle plus logique ?
...
Le sous-ensemble des listes Python est un jeu d'enfant. Prenez l'exemple de code ci-dessous, qui crée une liste `x` et y sélectionne « b ». Rappelez-vous qu'il s'agit du deuxième élément, et qu'il a donc l'index 1. Vous pouvez également utiliser l'indexation négative.
...
Vous vous souvenez de la liste `areas` de tout à l'heure, contenant à la fois des chaînes de caractères et des nombres flottants ? Sa définition figure déjà dans le script. Pouvez-vous ajouter le code correct pour effectuer un sous-ensemble Python ?
...
- Le nombre représentant la surface du salon est le 6ème élément de la liste, vous aurez donc besoin de `[5]` ici. `area[4]` afficherait la chaîne de caractères.
...
Ex().has_printout(0, not_printed_msg = "Reconsidérez votre code pour afficher le deuxième élément dans `areas`, qui est à l'index `1`.")
Ex().has_printout(1, not_printed_msg = "Reconsidérez votre code pour afficher le dernier élément dans `areas`, qui est à l'index `-1`.")
Ex().has_printout(2, not_printed_msg = "Reconsidérez votre code pour afficher la surface du salon. Elle est à l'index `5`.")
success_msg("Bon travail !")
```

---

## Trancher et découper en dés
...
Ex().has_printout(0, not_printed_msg="Avez-vous affiché `downstairs` après l'avoir calculé ?")
Ex().has_printout(1, not_printed_msg="Avez-vous affiché `upstairs` après l'avoir calculé ?")
...
- Vous pouvez étendre le site `areas_1` en y ajoutant des données sur votre garage. Ajoutez la chaîne de caractères `"garage"` et le flottant `15.45`. Nommez la liste résultante `areas_2`.
...
msg = "Ne supprimez pas et n'éditez pas la liste prédéfinie `areas`."
Ex().check_object("areas", missing_msg = msg).has_equal_value(incorrect_msg = msg)
Ex().check_object("areas_1").has_equal_value(incorrect_msg = "Utilisez `areas + [\"poolhouse\", 24.5]` pour créer `areas_1`. Attention aux fautes de frappe !")
Ex().check_object("areas_2").has_equal_value(incorrect_msg = "Utilisez `areas_1 + [\"garage\", 15.45]` pour créer `areas_2`. Attention aux fautes de frappe !")
...
Malheureusement, le montant que vous avez gagné à la loterie n'est pas si élevé et il semble que vous ne pourrez pas construire l’abri de piscine. Vous devrez le supprimer de la liste. Vous décidez de supprimer la chaîne de caractères et le nombre flottant correspondants de la liste `areas`.
...
Actuellement, le premier élément de la liste `areas_copy` est modifié et la liste `areas` est imprimée. Si vous appuyez sur le bouton d'exécution du code, vous constaterez que, bien que vous ayez modifié `areas_copy`, la modification est également prise en compte dans la liste `areas`. En effet, `areas` et `areas_copy` renvoient à la même liste.
...
imsg = "Assurez-vous de modifier UNIQUEMENT la copie, et non la liste originale `areas`. Consultez à nouveau la description de l'exercice si vous n'êtes pas sûr de savoir comment créer une copie."
Ex().check_correct(
